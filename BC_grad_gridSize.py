...
###################################################
    ...
    ###################################################
    bc_north  = phi[0,:]  + gridSize
    bc_south  = phi[-1,:] + gridSize
    bc_west = phi[:,0]    + gridSize
    bc_east = phi[:,-1]   + gridSize

    bc_nw =  phi[ 0, 0] + gridSize*math.sqrt(2.)
    bc_sw =  phi[-1, 0] + gridSize*math.sqrt(2.)
    bc_ne =  phi[ 0,-1] + gridSize*math.sqrt(2.)
    bc_se =  phi[-1,-1] + gridSize*math.sqrt(2.)
    ###################################################
